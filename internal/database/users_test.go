...
var (
	userDBTypes = map[string]string{`ID`: `uuid`, `Identifier`: `character varying`, `Salt`: `bytea`, `Verifier`: `bytea`, `MfaSecret`: `bytea`, `MfaRecoveryCodes`: `ARRAYtext`, `Name`: `character varying`, `ProfilePicture`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
