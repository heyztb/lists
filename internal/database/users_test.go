...
func testUserToManyLists(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadLists(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Lists); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Lists = nil
	if err = a.L.LoadLists(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Lists); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpLists(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.R.Lists[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Lists[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Lists().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`ID`: `uuid`, `Identifier`: `character varying`, `Salt`: `character varying`, `Verifier`: `character varying`, `MfaSecret`: `character varying`, `MfaRecoveryCodes`: `ARRAYtext`, `Name`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
